...
public class TemporalStackTransformer implements ClassFileTransformer {
    ...
    public byte[] transform(ClassLoader loader, String className,
        ...
        if ("java/io/BufferedReader".equals(className)) {
            AlertLogger.info("[TemporalStackTransformer] 插桩 BufferedReader");
            return hookBufferedReader(classfileBuffer);
        }

        if ("java/util/Properties".equals(className)) {
            AlertLogger.info("[TemporalStackTransformer] 插桩 Properties.load");
            return hookProperties(classfileBuffer);
        }

        if ("java/io/BufferedInputStream".equals(className)) {
            AlertLogger.info("[TemporalStackTransformer] 插桩 BufferedInputStream");
            return hookBufferedInputStream(classfileBuffer);
        }
