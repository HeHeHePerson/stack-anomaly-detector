...
if (action != null) {
    try {
        ...
        } else if ("set_threshold".equals(action)) {
            String type = request.getParameter("type");
            String value = request.getParameter("value");
            if ("param_length".equals(type)) {
                int v = Integer.parseInt(value);
                if (v >= 100 && v <= 1000) {
                    UrlBaselineModel.setParamLengthThresholdPercent(v);
                    msg = "参数值长度阈值已设置为 " + v + "%";
                } else {
                    msg = "参数值长度阈值需在 100-1000 之间";
                }
            } else if ("freq".equals(type)) {
                double v = Double.parseDouble(value);
                if (v >= 1.0 && v <= 10.0) {
                    UrlBaselineModel.setFrequencyThresholdMultiplier(v);
                    msg = "频率阈值已设置为 " + v + "x";
                } else {
                    msg = "频率阈值需在 1.0-10.0 之间";
                }
            }
        }
    } catch (Exception e) {
...
<div id="tab-url" class="tab-content">
  ...
  %>
  <p style="color:#888">共 <%= sortedUrls.size() %> 条基线路径（总请求数: <%= UrlBaselineModel.getTotalUrlsLearned() %>）
  &nbsp;|&nbsp; 长度阈值: <b><%= UrlBaselineModel.getParamLengthThresholdPercent() %>%</b>
  &nbsp;|&nbsp; 频率阈值: <b><%= String.format("%.1f", UrlBaselineModel.getFrequencyThresholdMultiplier()) %>x</b></p>
  <div style="margin-bottom:8px">
    <form method="post" style="display:inline">
      <input type="hidden" name="action" value="set_threshold">
      <input type="hidden" name="type" value="param_length">
      长度阈值%: <input name="value" size="4" value="<%= UrlBaselineModel.getParamLengthThresholdPercent() %>">
      <button type="submit" class="tab-btn" style="display:inline">设置</button>
    </form>
    &nbsp;
    <form method="post" style="display:inline">
      <input type="hidden" name="action" value="set_threshold">
      <input type="hidden" name="type" value="freq">
      频率阈值x: <input name="value" size="4" value="<%= String.format("%.1f", UrlBaselineModel.getFrequencyThresholdMultiplier()) %>">
      <button type="submit" class="tab-btn" style="display:inline">设置</button>
    </form>
  </div>
  <div style="max-height:70vh;overflow-y:auto">
  <table>
    <tr><th style="width:30px">#</th><th>路径</th><th style="width:70px">访问次数</th><th style="width:80px">速率/min</th><th>参数名</th><th>值最大长度</th><th style="width:50px"></th></tr>
    <%
      int uIdx = 0;
    ...
    <tr>
      ...
      <td><%= ub.paramMaxLengths.toString() %></td>
      <td>
