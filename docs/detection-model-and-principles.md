...
3. 记录或更新 UrlBaseline：
   ...
   - 提取参数值并计算长度 → 更新 paramMaxLengths（仅保留最大值）
...
```
学习期访问: /examples/index.html?lang=zh&q=abcd → 基线: path=/examples/index.html, params=[lang, q], maxLen: lang=2, q=4

检测期:
  /examples/index.html              → 已知路径，无参数 → 静默
  /examples/index.html?lang=en      → 已知路径，已知参数，长度未超阈值 → 静默
  /examples/index.html?cmd=whoami   → 已知路径，新参数 cmd → [URL] 新参数告警
  /examples/index.html?q=<2KB_shellcode> → 已知路径，已知参数，值长度 2048 > 4×150%=6 → [URL] 参数值长度超阈值告警
...
(按访问次数降序)
  ...
  [  1] /examples                        访问=1次 (2.0/min)  参数=[]  值最大长度={}
  [  2] /examples/servlets               访问=1次 (2.0/min)  参数=[]  值最大长度={}
  [  3] /examples/index.html             访问=1次 (2.0/min)  参数=[lang, q]  值最大长度={lang=2, q=4}
  ...
```
...
| 阈值 | 启动参数 | 默认值 | 管理控制台设置 |
|------|---------|--------|---------------|
| 频率阈值 | `url.freq.threshold` | 1.5 (150%) | URL 标签页可在线修改 |
| 参数值长度阈值 | `url.param.threshold` | 150 (%) | URL 标签页可在线修改 |

阈值范围限制：
- 频率阈值: 1.0 ~ 10.0（超出范围使用默认值）
- 参数值长度阈值: 100 ~ 1000（超出范围使用默认值）
...
| URL Profile 列表 | 展示学习到的所有 URL 路径、访问次数、基线速率、参数名集合和参数值最大长度 |
| URL 路径移除 | 从 BASELINE 中移除指定路径，后续访问该路径将触发 `[URL] 新URL首次出现` 告警 |
| 阈值在线设置 | 支持在线调整频率阈值（1.0-10.0x）和参数值长度阈值（100%-1000%），即时生效 |
